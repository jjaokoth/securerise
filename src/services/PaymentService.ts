...
export class PaymentService {
  ...
  async createHandshake(input: CreateHandshakeInput) {
    ...
    const created = await this.prisma.payoutHandshake.create({
      data: {
        ...
        // Safety net PoD binding not yet available
  ...
  async verifyAndRelease(input: VerifyAndReleaseInput) {
    ...
    const safetyNetUrl = assertNonEmptyString(input.safetyNetImageUrl, 'safetyNetImageUrl');
    const gpsCoords = input.gpsMetadata;

    // Safety net logging (audit)
    // eslint-disable-next-line no-console
    console.info('[Securerise] SafetyNet verify', {
      handshakeId,
      safetyNetUrl,
      gpsCoords,
      idempotencyKey,
    ...
    const gpsCiphertext = JSON.stringify(gpsCoords);

    // Idempotency: single atomic release with idempotency key.
    // If another concurrent request uses the same key, we should treat as already processed.
    try {
      const result = await this.prisma.$transaction(async (tx) => {
        ...
        // OTP check
        if (handshake.handshakeCodeHash !== userOtpHash) throw new Error('INVALID_OTP');

        // If already released, prevent double payout.
        ...
        // Release transition
        const updated = await tx.payoutHandshake.update({
          where: { id: handshake.id },
          data: {
            status: 'RELEASED',
            releaseIdempotencyKey: idempotencyKey,
            podPhotoUrlCiphertext: safetyNetUrl,
            podGpsCiphertext: gpsCiphertext,
            releasedAt: new Date(),
          },
        });
